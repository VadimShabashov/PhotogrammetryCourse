...
//            std::cout << "gauss: unique solution found" << std::endl;
    ...
    cv::Mat estimateHomographyRANSAC(const std::vector<cv::Point2f> &points_lhs, const std::vector<cv::Point2f> &points_rhs)
    {
        ...
        double get_inlier_prob = 0.5;
        const int n_trials = std::ceil(std::log(1 - success_prob) / std::log(1 - std::pow(get_inlier_prob, n_samples)));
...
{
    cv::Mat res_hom_coord = T * cv::Mat({ pt.x, pt.y, 1.0 });
    double res_w = res_hom_coord.at<double>(0, 2);

    if (std::abs(res_w) < 0.0001) {
        return { 0.0, 0.0 };
    }

    double x = res_hom_coord.at<double>(0, 0);
    double y = res_hom_coord.at<double>(0, 1);

    return { x / res_w, y / res_w };
}
